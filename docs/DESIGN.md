...
| `regression` | Mean time for a key under the newest app version **that is still running** — so a rollback is not reported as the regression it undid — is more than 3× the previous version's and at least 20 ms slower, with 100 or more calls on each side | Both versions' timings, the DB finding for the key, and any DB finding that appeared between the two deploys, other than statements and Info | High, Critical above 10× |
| `transaction_held_open` | p95 open time above 500 ms and DB time inside below 30 percent of it, over at least 20 transactions | DB side: a session idle in transaction whose last statement this operation runs, or a session blocked on a lock whose holder last ran one | Medium, High when such a session is reported. An unrelated idle session is not the database agreeing |
