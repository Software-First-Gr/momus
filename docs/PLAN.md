...
| 2026-09-24 | **Control-room Phase 0 started (`docs/CONTROL-ROOM.md`): the first real target is the author's staging database, on a SQL Server it shares with a demo database running the same application.** Preparing it found eight things the Postgres demo could not show. Fixed: the image could not scan SQL Server at all (Alpine's invariant globalization; `843eb78`), statements and sessions were read instance-wide (D24), and idle background waits were most of every wait report. Live SQL Server tests exist for the first time (`MOMUS_TEST_MSSQL`), each proven to fail without its fix. Later the same day, findings 2 and 3 (D25, D26): WebSocket and event-stream requests are no longer one operation, a statement that outlived its request no longer fails the application's query, and a server on another machine takes a key and a port of its own. Then findings 4–6 (D27–D29): a build with no revision reports a fingerprint of its assemblies, the SQL Server grant is `VIEW SERVER STATE` and a bare database user, and an application's database is matched to a scanned target by provider and name. All eight Phase 0 findings are closed. Next: M1's week against staging. |
| 2026-09-12 | **Monday to Thursday of the week of 14 September done, and M3's "done when" met.** D3 decided: Apache-2.0 for everything. The first release is `v0.1.0` (D18). The database side is ranked on the last hour and leaves Momus's own statements out (D19); Fix first ranks by reach (D20); the demo can deploy a regression and exercises all six rules. 223 + 22×3 tests. Next: Friday — `momus serve` beside a real database, which starts M1's week and gates `v0.1.0`. |
| 2026-09-07 | **M3 built, not released.** Deploys, `regression`, transactions and pool waits, the "Fix first" ranking, evidence packs, mute and fixed, the History tab and a nightly vacuum. Plus a Diagnostics tab that was not on any list: a two-sided tool fails silently, and every one of those failures looks like "nothing to report" everywhere else. 171 + 22×3 tests. **`v0.3.0` is deliberately not tagged** — a tag publishes to nuget.org and GHCR, D3 is still open, and `v0.1.0` and `v0.2.0` never happened either. The release checklist is a separate decision from the code. |
| 2026-09-07 | **M2.4 done: the engine says what the two halves mean.** `n_plus_one`, `hot_query_origin` and `db_finding` run every 15 s over a snapshot of the store, and the Insights tab leads the nav. On the demo: *GET /api/orders/{id:int} runs one statement ×6 per call (Workload.cs:109), 326 times a minute* — the shape, the cost and the line of code in one sentence. **Call sites did not work at all before this session and now do**; see D16, it is the whole of D6's promise. 138 tests. Next: M2.5 is the `v0.2.0` release, or M3 if the release waits for M1's week beside a real database. |
...
- [x] `pg_stat_statements` and `dm_exec_query_stats` are cumulative since the last reset; the app side is the last 60 minutes. Keep the previous snapshot per statement and rank on the delta between scans, so "#3 by total time" means this hour. On the demo, two seed INSERTs from five days earlier, one call each, were still a High and a Medium hot query, and one of them held a Fix first slot. Handle a stats reset and a statement evicted from the view (a negative delta) explicitly.
- [x] Momus's own statements out of the Queries tab: about 40 of its 50 rows were check SQL, extension DDL and `DISCARD ALL`. Mark the checks' queries (a leading `/* momus */` comment, for example) and exclude them; keep utility statements out of `hot_query_origin`. Done by fingerprint instead of a comment, see D19.

**Wed — ranking**

- [x] `InsightRanking.Share` gives a `server` subject the whole of the traffic and a session the 0.02 floor. Observed: a Low buffer-cache card at #2 of Fix first, and a new High idle-in-transaction at #5 below an Info hot query. A subject with no traffic of its own should be neutral, not scaled to everything or to nothing. Pin both orderings as tests, then update DESIGN.md's Ranking section. Done as reach, `1 + 2 × share`, neutral 2 without traffic (D20).

**Thu — the demo exercises all six rules**

- [x] A version knob for the shop (build argument into `InformationalVersion`) and a "slow build" switch, so `regression` can fire — then time M3's "done when" for real. Everything in the demo reports 0.0.1, so it has never been shown. Done as the `SHOP_VERSION` and `SHOP_SLOW_BUILD` build arguments; the slow build holds a lock on `carts`, so the cart update gets slower with its SQL unchanged. **M3's "done when" is met:** on 2026-09-12 the 1.1.0 slow build started at 20:43:44 and "A statement in POST /api/cart/{id:int}/items got 24.7× slower in 1.1.0" (Critical) was on the Insights tab 40 s later.
- [x] `idle_transaction` and `connection_flood` open raw `NpgsqlConnection`s, which `Momus.Client` cannot see, so `transaction_held_open` and `pool_wait` never fire in the demo. Add EF Core variants. Done as two new scenarios, `slow_checkout` and `pool_exhaustion`; both insights fired within 30 s of starting them.
...
**Sat — what Thursday's run found**

- [x] `transaction_held_open` borrowed a High from an unrelated idle session. Now joined by fingerprint (D22).
- [x] `regression`'s "started reporting between the two deploys" offered "0.1 ms across 2 calls". Statements and Info are left out.
- [x] `pool_wait` named one operation twice ("POST /api/checkout/{id:int} and POST /api/checkout/{id:int}"): transactions are one row per call site, and are now added up per operation.
- [x] p95 read as a bucket edge ("holds a transaction open for 1,024 ms" for 700 ms). `Timing.Percentile` interpolates within the bucket and never exceeds the largest value seen.
- [x] Postgres had no lock-wait check, so the slow build's lock contention was invisible from the database side. `pg.lock_waits` names the blocked session, the holder and what the holder last ran; verified on the demo with an idle-in-transaction holder, found 23 s after the chain formed. It cannot see sub-second contention (D22).
- [x] A rollback kept its regression card (D21).
- Short windows ("in the last 30 s") only happen on a store with no earlier counters — a first run, or the day `statement_samples` was added. Not a bug; left alone.
...
| D8 | 2026-09-06 | Registry: GHCR (`ghcr.io/software-first-gr/momus`), `edge` from `develop`, semver + `latest` from tags, linux/amd64 + linux/arm64. Docker Hub `momus` namespace is taken; a mirror is optional later. **Superseded by D23.** |
...
| D19 | 2026-09-12 | **The database side is ranked on what changed between scans, by the server; the checks stay cumulative and stateless.** `pg_stat_statements` and `dm_exec_query_stats` count from their last reset, the app side is the last hour, and a check has no memory — the CLI has to keep working without one. So the server reads 500 statements a scan, keeps their counters in `statement_samples` for three hours, subtracts the newest scan at least an hour old (the oldest it has when younger, never one older than two hours), and keeps the 50 that did the most in that window. Titles and evidence say which window they cover; a first scan with nothing to subtract says "since statistics were reset". Counters that went down mean a reset or an eviction, so everything they hold counts. A statement missing from the baseline counts from zero — right for a new one, an overstatement for one that was merely below the 500 cut-off, which is the cost accepted. Session bookkeeping (`DISCARD ALL`, `SET`, `BEGIN`, …) is sampled but never ranked. **Momus's own statements are recognised by fingerprint, not by a comment marker**, as this week's plan had suggested: Postgres ignores comments when it groups statements, so a database scanned before the marker existed keeps showing the unmarked text. `Db` remembers the fingerprint of everything it sends, and the top-queries checks run last so the one-shot CLI already knows them. Verified on the demo: the Findings tab went from 50 lifetime-ranked statements — about 40 of them Momus's own reads, extension DDL, `DISCARD ALL` or days-old seed data — to the 7 that ran in the last minute, with none of Momus's own left on the Queries tab. |
| D20 | 2026-09-12 | **Ranking multiplies by reach, `1 + 2 × share`, with a neutral 2 for subjects that carry no traffic.** The old factor was the raw share, clamped to 0.02–1, with `server` and `database` counted as all of the traffic and everything else without traffic as the floor. Both ends were wrong on the demo's Fix first: "Buffer cache hit ratio is 98.9 %" (Low) was second, above the N+1 on the busiest endpoint and a High sequential scan; a transaction idle for five minutes (High, new, on a `session`) was fifth, below an Info hot query. A server or a session is not something an application calls, so traffic says nothing about it either way — hence neutral, not everything and not nothing. Reach runs 1–3 rather than 1–2 so DESIGN's own example survives: a Critical nothing calls (16 × 1.02) still loses to a High on the busiest endpoint (8 × 2.98). Both demo orderings are pinned as tests in `RankingTests`. |
| D21 | 2026-09-13 | **`regression` judges the newest version that is still running, not the last one to appear.** "Newest" was the version first seen most recently, so after a rollback the card kept saying "got slower in 1.1.0" about a build nobody was running. A version counts as running if it reported within two minutes of the latest report (`RunningGrace`); among those the most recently deployed is judged, against whichever other version ran most recently. A rolling deploy therefore judges the new version, and a rollback judges the version rolled back to against the one taken away. Verified on the demo: after rolling 1.1.0 back to 1.0.0 the Critical card was gone within 135 s of the rollback starting. |
| D22 | 2026-09-13 | **A session finding is the database "agreeing" with an app-side insight only through a fingerprint.** `transaction_held_open` went to High whenever any idle or blocked session existed; on the demo an unrelated idle session promoted a 700 ms checkout and was quoted as agreement. Session findings now record the fingerprint of the statement the session last ran (`query_fingerprint`), and a new check, `pg.lock_waits`, records the blocker's (`blocker_query_fingerprint`) — the waiting statement is stored under a different key, because being blocked is not holding anything open. A session counts only when one of those is a statement the same operation runs. SQL Server's blocking check does not record fingerprints yet, so it never counts rather than always counting. `pg.lock_waits` is point-in-time like every view it reads: it finds blocking chains of five seconds or more, not the tens-of-milliseconds contention of the demo's slow build, which the app-side `regression` is what catches. |
...
| D23 | 2026-09-24 | **The image moves to Docker Hub as `softwarefirst/momus`, public; GHCR is dropped. Supersedes D8.** The product has to be plug and play, and Docker Hub is the registry `docker` assumes: `docker run softwarefirst/momus` needs no registry host and no login. D8's two objections were gone: the `softwarefirst` organization already exists and is paid for, so the taken bare `momus` namespace does not matter. The cost accepted is a stored credential where GHCR needed none: an organization access token that can push to this one repository only, in the `DOCKERHUB_TOKEN` secret, with the organization name in `DOCKERHUB_USERNAME`. Tags are unchanged: `edge` from `develop`, semver and `latest` from `v*` tags, linux/amd64 and linux/arm64. The GHCR package that `edge` had been pushed to since 2026-09-07 was private, which is GHCR's default, so nobody could pull it and the README's "no image has been pushed" had been read as true. Delete it rather than leave a second, stale copy. |
| D24 | 2026-09-24 | **A scan reports the statements, sessions and blocking of the database it is connected to, not of the instance; through a system database it keeps the instance.** SQL Server's `dm_exec_query_stats` and `dm_exec_requests` and Postgres's `pg_stat_activity` cover every database on the server, and only `pg_stat_statements` was filtered. Two databases running one application on one server — staging beside a demo, the first real target's layout — reported each other's statements under identical fingerprints, and since D22 each other's sessions as agreement. SQL Server filters top queries on the plan's `dbid` attribute (`sys.dm_exec_plan_attributes`): `dm_exec_sql_text.dbid` is NULL for prepared statements, which is everything EF Core sends — measured, one prepared statement run in two databases gave two cache rows, both NULL there, 5 and 6 in the plan attribute. Blocking filters on the waiting request's `database_id`; Postgres's session and lock-wait checks on `datname`, leaving the lock holder unfiltered, because whoever holds the lock is the answer. A connection to `master`, `tempdb`, `model` or `msdb` has no application of its own, so it keeps the instance-wide view, and the evidence now names each statement's database. Waits, page life expectancy, cache hit ratio and connection saturation are the instance's by nature and say so. On the author's development instance the change removed the old #1 and #2 by CPU, both in `master`, and a statement from another application database. |
| D25 | 2026-09-24 | **A WebSocket or an event stream is not an operation, and an operation that has ended is not current.** `MomusMiddleware` opened one operation per request, and a Blazor Server circuit is one WebSocket for the life of its tab: measured, with a socket and an event stream held open the client had reported none of their statements, and closing one would have delivered the whole session as a single request, every repeated lookup a loop to `n_plus_one`. Such requests now open no operation, and their statements take the ambient path, named after the innermost `Activity`. The check reads the handshake — `Upgrade: websocket`, an HTTP/2 extended CONNECT, `Accept: text/event-stream` — because the middleware runs before `UseWebSockets` and until then a WebSocket request does not say it is one. The test for work outliving its request found the worse bug: the operation's name was resolved on first use, a late statement asked for it after the `HttpContext` was disposed, and the `ObjectDisposedException` came out of the application's own query. The name is now settled when the operation ends, an ended operation is skipped by `Current` (falling back to any enclosing one still open), and every interceptor callback catches its own exceptions, counts them, and has the exporter log them — the client must never be why a query fails. gRPC streaming is left as one operation per call: nothing in the request says it will be long. |
| D26 | 2026-09-24 | **A server on another machine takes a key and a port of its own; the pages stay unauthenticated and on loopback.** The defaults assumed the application and the server share a machine, and off it anything that could reach the port could post windows, read query text and add targets through Settings. `MOMUS_INGEST_KEY` makes `POST /api/v1/ingest` require the same value in the `X-Momus-Key` header, which the client sends from `Momus:IngestKey`; the comparison is of SHA-256 hashes in constant time. `MOMUS_INGEST_PORT` opens a second port that answers only ingest and `/healthz`, judged by the port the connection arrived on and never by the Host header, which the caller writes — so `-p 127.0.0.1:4848:4848 -p 4849:4849` is the whole deployment, with no proxy. The key is environment-only, never a flag, to stay out of `ps` and shell history. A refused window is a Problem on Diagnostics with the count and the last address, and the client logs "refused" rather than "not answering", because a wrong key looks like a stopped application everywhere else. A key does not protect the pages: a browser cannot send a header, and a login is Pro's (D10). Both guards were mutation-tested: judging the port by the Host header, and not sending the key, each failed exactly the test written for it. |
| D27 | 2026-09-24 | **When the informational version names no revision, the client reports a build fingerprint with it.** A deploy is a version the server has not seen, and the SDK only writes the commit when it can see `.git` — which most Dockerfiles exclude, the first real target's among them — so a container build reported `1.0.0` after every deploy and `regression` could never fire. The client now appends `+build.<12 hex>`: SHA-256 over the module version id of every managed assembly in the application's directory, in name order, plus the runtime version. Measured on .NET 10 before relying on it: a clean rebuild of unchanged source gives identical module ids, and a body-only change in a referenced library moves that library's id but not the application's, because the application compiles against the library's reference assembly — so hashing the entry assembly alone would have missed most changes. Package upgrades change it too, which is right: they are deploys. On the first target's build output (225 assemblies, 607 MB) it costs 99 ms once, on the exporter's thread, reading metadata without loading anything. A version that already names a revision is left alone, and Diagnostics warns about any application whose version still names none. Chosen over asking applications to pass `SourceRevisionId`, which stays documented as the way to see the commit instead. |
| D28 | 2026-09-24 | **The SQL Server grant is `VIEW SERVER STATE` and a database user with no permissions; the missing-index check names tables from the DMV.** Measured on 2022 with a scratch login: with `VIEW SERVER STATE` alone the login cannot open the database and no check runs (the README had stopped there); with a user in the database every check runs, but `OBJECT_NAME` needs metadata visibility, so the missing-index finding read "Optimizer wants an index on  (est. 99% improvement)" — keyed to a table with no name, joining nothing. `sys.dm_db_missing_index_details.statement` holds `[db].[schema].[table]` and is readable under the same grant, so the check parses it with `PARSENAME` and `VIEW DEFINITION` is not needed. On 2022, `VIEW SERVER PERFORMANCE STATE` plus the user ran all five checks too. A scan failing with "Cannot open database" now tells the reader to create the user. The live test proves each level and fails on the old `OBJECT_NAME` query. |
| D29 | 2026-09-24 | **An application's database is matched to a scanned target on provider and database name when the ids differ.** The client names a target by the slug of its database's name; a target from the environment or Settings by the slug of whatever name it was given. When the client shares its connection string the ids agree by construction, but a server on another machine is never handed one, so the two halves joined only by coincidence of naming — the reason Phase 0's instructions had to say `MOMUS_TARGETS__0__NAME` must equal the database name. `TargetMatch.Resolve` (pure) rewrites a batch target the server does not know by id to the one known target with the same provider (spellings normalised: `mssql` is `sqlserver`) and database name — from its last scan, or else from its connection string's `Database` / `Initial Catalog`, so it works before the first scan — and never guesses between two. It runs before the batch is stored, so the shared-connection-string path no longer registers a second target for a database already scanned under another name either. |
...
**Goal.** `momus serve` runs beside a database, scans it on a schedule, keeps every scan in SQLite, and shows findings with first/last seen on one page. Docker image on Docker Hub. This alone beats the CLI for daily use because it has memory.
...
- [x] **One operation, several threads (2026-09-24).** A request that runs two contexts at once — `Task.WhenAll` over an `IDbContextFactory` — carries one `OperationContext` in its execution context, and the command, transaction and connection interceptors wrote its plain `Dictionary`, `List`s and counters from both threads at the same moment. `ConcurrencyTests` runs eight contexts inside one `MomusOperation.Begin`, each doing 25 × (a transaction around a lookup, a count, a statement new to the operation), for 20 rounds, and checks every count, row total, histogram, transaction and connection acquisition against what ran, with no exception caught inside the client. Before the fix it failed in each of 15 runs across the three majors, with 18 to 20 of the 20 rounds wrong. In about one run in fifteen, a map corrupted by two inserts at once made `Complete` throw a `NullReferenceException` out of `MomusOperation`'s `Dispose` and into the application: D25's guards wrap the interceptor callbacks, not `Complete`'s two callers. Fixed with one private lock per operation around `Record`, `AddRows`, `RecordTransaction`, `RecordPoolWait`, `DbMsSoFar` and `Complete`. `Complete` also copies the histograms, so a statement recording just after the snapshot cannot change it. Uncontended, which is the usual case of one thread per operation, the lock adds about 3 ns to a 17 ns `Record` (microbenchmark; `Monitor` and .NET 9's `Lock` cost the same, so there is no `#if`). Mutation-tested: removing any one of the four recording locks, or the copy, fails its test in three runs of three.
...
- [x] Tests: a `Momus.Client.Tests` project multi-targeting net8.0/net9.0/net10.0 that asserts `AddMomus()` alone causes an interceptor to fire on all three EF majors — with `AddDbContext`, `AddDbContextPool` and `AddDbContextFactory`, and *not* when it is called before them. 18 tests × 3 majors, against SQLite, which travels the same `DbCommandInterceptor` path as Npgsql with no server to run. Per D16 it also asserts a real call site: the sample application lives in namespace `SampleApp`, deliberately outside `Momus.Client.*`, because a test whose application code sat inside that tree could never prove the walker names application code rather than the framework above it. `RollForward` lets the net8/net9 hosts run on a developer machine that only has the newest runtime; CI now installs all three so the runtime half is proved too.
  - Two more bugs, both found by writing these tests and both silent in production. **The call-site filter matched raw prefixes**, so `"Npgsql"` also swallowed an application namespace called `NpgsqlHelpers` and `"Momus.Client"` swallowed everything under it; it now matches on namespace boundaries. **Row counts were all one too high**: EF's `ReadCount` counts the read that finds the end, and measuring seven query shapes on all three majors — `FirstOrDefaultAsync` hit and miss, `ToListAsync` of three and of none, `SingleOrDefaultAsync`, `CountAsync`, `AnyAsync` — showed it is rows + 1 in every one, including the ones that look like they stop early. A `[Theory]` pins all of them, so a change in EF's enumeration is a failing test rather than a number that is quietly wrong everywhere.
...
- [x] `IInsight` and `IInsightContext` in `Momus.Core` (contracts only); implementations in `Momus.Server/Insights`. The context is a **snapshot** loaded before any rule runs, not a set of reads a rule may issue: that makes a rule a pure function of its inputs, keeps two rules from disagreeing about the numbers mid-evaluation, and is why `QueryStatView` carries its rates rather than computing them off the clock the way the store's model does. It declares what M2's rules need and nothing more; M3 adds version timings, transactions and pool waits when it has something to use them. DESIGN.md updated.
- [x] `insights` table upserted by stable key (kind + subject) with first_seen, last_seen, status. Subjects alone were not enough: cache hit ratio and connection saturation are both about `server` and nothing else, so `Insight.Discriminator` carries the check id for pass-through rows.
- [x] `hot_query_origin` and `n_plus_one` per the rules in DESIGN.md, with the thresholds of D15. `db_finding` passes through everything that is *not* about a single statement and names the endpoints that touch its table.
- [x] Insights tab, ordered by severity then last seen, first in the nav. It says which half is missing rather than looking empty: with no client reporting, "every insight below comes from the database alone".
- [x] Tests: 13, on hand-built snapshots — every rule fires on a crafted dataset and stays quiet on a healthy one — plus 4 on an in-memory store for the lifetime and identity behaviour.
- [x] **Decided: D15.** The demo's N+1 repeats six times, so the ×10 rule would have stayed silent on the one dataset built to trigger it.

**Two things found by running it, not by a test.** Both are in the Decisions table because both change what the product does: call sites were captured after every `await` and were therefore always null (D16), and insight prose was formatted in the machine's locale, so a Greek machine rendered "2.543 times a minute at 0,2 ms" inside an English sentence. The server now pins invariant culture and the rules format invariantly — the same text ends up in M3's evidence packs, which are meant to be pasted into a coding agent. DESIGN.md says "more than 10 times inside a single operation", and `samples/Shop.Api`'s deliberate N+1 repeats **six** times — an order has about six lines — so the rule as written stays silent on the one dataset built to trigger it. Either the threshold is wrong (an endpoint issuing 8 statements per call is an N+1 at 6 as much as at 40), or the sample should generate fatter orders. Do not "fix" this by tuning the number until the demo lights up: pick the one that is true of real applications, then make the demo match it. The number the client reports is `MaxRepeatsPerOperation`, already stored per window.
...
- [x] `deploys` derived from `app_instances` (first seen per version). Header shows the current version, on every page, from one scoped read.
- [x] `regression`: mean ms under newest version > 3× previous and ≥ 20 ms slower, ≥ 100 calls each side; Critical above 10×. Evidence includes DB findings that appeared between the two deploys — the candidate explanation nobody thinks to go looking for.
- [x] Client: `MomusTransactionInterceptor` (open ms, DB ms inside, call site) and `MomusConnectionInterceptor` (acquisition time). Contract additions `transactions`, `pool`, and `client`. Both are timed by Momus rather than read from EF's `Duration`, which means one thing on a start event and another on a commit event, and both are measured in `Momus.Client.Tests` against a transaction held open for a known interval.
- [x] `transaction_held_open` (p95 open > 500 ms and DB share < 30%) joined with idle-in-transaction / blocking findings. `pool_wait` (p95 > 100 ms) joined with connection saturation, and naming the operations holding transactions open — which is nearly always the cause one step earlier.
- [x] Ranking score: severity weight × share of traffic hitting the subject × recency factor. Top five are the "Fix first" home page, with sparkline, call-site chip and DB-evidence chip. Findings moved to `/findings`.
  - The share is the **widest** subject, not the sum. Found on the running demo, where a Low unused index outranked a High sequential scan on the same table: the unused-index finding carries `index:` and `table:`, and summing them scored it as if it were about the whole application.
- [x] Evidence pack: Markdown on a copy button with both sides' numbers, the normalized query, and the versions — with a test asserting no connection string can reach it, since the whole point is that it gets pasted somewhere else.
- [x] Mute and Fixed actions; a fixed insight that fires again reopens and is marked as having done so.
